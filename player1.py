...
class Drone(object):
  ...
  pos = Point() # position of the drone
...
class Zone(object):
  ...
  the ownerid"""
  center = Point() # center of the zone (circle radius  = 100 units)
  ownerId = -1 
...
class Game(object):
  ...
  def init(self):
    ...
    for areaId in xrange(z):
      ...
      Game.zones.append(z1) # add the zone to the list of zones
    ...
    for teamId in xrange(p):
      ...
      Game.teams.append(t1[teamId]) # add each team to the teams list 
  ...
  def play(self):
    ...
    myDrones = Game.teams[Game.myTeamId].drones
